...
namespace Hazel {
	...
	Ref<Scene> Scene::Copy(Ref<Scene> other)
	{
		...
		// Copy components (except IDComponent and TagComponet)
	...
	void Scene::OnRuntimeStart()
	{
		...
		for (auto e : view)//Runtime启动后,遍历所有Entity,如果附带了Rigidbody2DComponent,将该entity的Rigidbody2DComponent组件Runtime设置为b2body
		{
	...
	void Scene::OnUpdateRuntime(Timestep ts)
	{
		// Update scripts
		{
			m_Registry.view<NativeScriptComponent>().each([=](auto entity, auto& nsc)//实体ID 和对应的组件引用
				{
					// TODO: Move to Scene:OnScenePlay
					if (!nsc.Instance)//当前组件未初始化
					{
